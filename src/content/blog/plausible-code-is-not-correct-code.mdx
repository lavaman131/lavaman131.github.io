...
```typescript
// Bad: God module the agent can't reason about
// user-service.ts (2,400 lines)
export class UserService {
  // Authentication, authorization, profile management,
  // notification preferences, billing integration,
  // activity logging, and data export all in one class
}

// Good: Focused modules the agent handles reliably
// auth.ts (~200 lines) - Authentication only
// permissions.ts (~150 lines) - Authorization only
// profile.ts (~300 lines) - Profile CRUD only
// notifications.ts (~200 lines) - Notification preferences only
```
