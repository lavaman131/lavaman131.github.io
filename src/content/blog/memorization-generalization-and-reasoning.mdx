...
<body>
    <div class="container">
        ...
        <div class="code-section">
            <button id="codeToggle" class="code-toggle">🔍 View Shannon Entropy Algorithm</button>
            <div class="code-display" id="codeDisplay">
<pre><span class="code-comment">// Shannon Information Theory - The Core Algorithm</span>
<span class="code-comment">// This calculates how many bits are needed to compress information</span>

<span class="code-function">updateCompression</span>() {
    <span class="code-comment">// Get probability as percentage (1-100%)</span>
    <span class="code-keyword">const</span> prob = <span class="code-function">parseInt</span>(probSlider.value);
    
    <span class="code-comment">// Convert to decimal probability (0.01-1.0)</span>
    <span class="code-keyword">const</span> decimal = prob / <span class="code-number">100</span>;
    
    <span class="code-comment">// 🎯 SHANNON ENTROPY FORMULA: -log₂(probability)</span>
    <span class="code-comment">// Lower probability = More surprise = More bits needed</span>
    <span class="code-keyword">const</span> cost = -<span class="code-function">Math.log2</span>(decimal);
    
    <span class="code-comment">// The difference in compression costs between models</span>
    <span class="code-comment">// reveals how much one model "memorized" vs the other</span>
    <span class="code-keyword">const</span> memorization = referenceModelCost - targetModelCost;
    
    <span class="code-comment">// Visual representation for the progress bar</span>
    <span class="code-keyword">const</span> width = <span class="code-function">Math.min</span>(cost * <span class="code-number">10</span>, <span class="code-number">100</span>);
}

<span class="code-comment">// Key Insight: If a model assigns high probability to text,</span>
<span class="code-comment">// it needs fewer bits to compress it = more memorization!</span>

<span class="code-comment">// Examples:</span>
<span class="code-comment">// 100% probability → 0 bits (perfect memorization)</span>
<span class="code-comment">// 50% probability → 1 bit (some memorization)</span>
<span class="code-comment">// 1% probability → ~6.6 bits (little/no memorization)</span></pre>
            </div>
        </div>
    ...
    <script>
        ...
        codeToggle.addEventListener('click', function() {
            if (codeDisplay.classList.contains('show')) {
                ...
                codeToggle.textContent = '🔍 View Shannon Entropy Algorithm';
            } else {
                codeDisplay.classList.add('show');
                codeToggle.textContent = '❌ Hide Shannon Entropy Algorithm';
            }
        });
    </script>
</body>
...
<body>
    <div class="container">
        ...
        <div class="controls">
            ...
            <button id="codeToggle" class="code-toggle">🔍 View Algorithm Code</button>
        </div>

        <div class="code-section">
            <div class="code-display" id="codeDisplay">
<pre><span class="code-comment">// This is the core recursive algorithm for solving Tower of Hanoi</span>
<span class="code-function">solveTowerOfHanoi</span>(<span class="code-keyword">n</span>, <span class="code-keyword">source</span>, <span class="code-keyword">destination</span>, <span class="code-keyword">auxiliary</span>) {
    <span class="code-comment">// BASE CASE: If there's only 1 disk, move it directly</span>
    <span class="code-keyword">if</span> (<span class="code-keyword">n</span> === <span class="code-number">1</span>) {
        <span class="code-keyword">this</span>.moves.<span class="code-function">push</span>({ 
            <span class="code-keyword">from</span>: <span class="code-keyword">source</span>, 
            <span class="code-keyword">to</span>: <span class="code-keyword">destination</span>, 
            <span class="code-keyword">disk</span>: <span class="code-number">1</span> 
        });
    } <span class="code-keyword">else</span> {
        <span class="code-comment">// RECURSIVE CASE: Break down the problem into 3 steps</span>
        
        <span class="code-comment">// Step 1: Move n-1 disks from source to auxiliary tower</span>
        <span class="code-comment">// (using destination as temporary storage)</span>
        <span class="code-keyword">this</span>.<span class="code-function">solveTowerOfHanoi</span>(<span class="code-keyword">n</span> - <span class="code-number">1</span>, <span class="code-keyword">source</span>, <span class="code-keyword">auxiliary</span>, <span class="code-keyword">destination</span>);
        
        <span class="code-comment">// Step 2: Move the largest disk from source to destination</span>
        <span class="code-keyword">this</span>.moves.<span class="code-function">push</span>({ 
            <span class="code-keyword">from</span>: <span class="code-keyword">source</span>, 
            <span class="code-keyword">to</span>: <span class="code-keyword">destination</span>, 
            <span class="code-keyword">disk</span>: <span class="code-keyword">n</span> 
        });
        
        <span class="code-comment">// Step 3: Move n-1 disks from auxiliary to destination</span>
        <span class="code-comment">// (using source as temporary storage)</span>
        <span class="code-keyword">this</span>.<span class="code-function">solveTowerOfHanoi</span>(<span class="code-keyword">n</span> - <span class="code-number">1</span>, <span class="code-keyword">auxiliary</span>, <span class="code-keyword">destination</span>, <span class="code-keyword">source</span>);
    }
}</pre>
            </div>
        </div>
    ...
    <script>
        class TowerOfHanoi {
            ...
            initializeEventListeners() {
                ...
                document.getElementById('codeToggle').addEventListener('click', () => this.toggleCode());
            }
            ...
            toggleCode() {
                ...
                if (codeDisplay.classList.contains('show')) {
                    codeDisplay.classList.remove('show');
                    toggleButton.textContent = '🔍 View Algorithm Code';
                } else {
                    codeDisplay.classList.add('show');
                    toggleButton.textContent = '❌ Hide Algorithm Code';
                }
            }
            ...
            async animateMove(move, moveIndex) {
                ...
                this.highlightCurrentMove(moveIndex);
            ...
            displaySolution() {
                ...
                this.moves.forEach((move, index) => {
                    ...
                    moveElement.id = \`move-\${index}\`;
                    moveElement.textContent = \`\${index + 1}. Move disk from Tower \${move.from} to Tower \${move.to}\`;
            ...
            highlightCurrentMove(index) {
                ...
                const previousMove = document.querySelector('.current-move');
                if (previousMove) {
                    previousMove.classList.remove('current-move');
                }
                ...
                if (currentMove) {
                    ...
                    // Only auto-scroll if the solution display is currently visible
                    // This allows users to scroll away without being forced back
                    const solutionDisplay = document.getElementById('solutionDisplay');
                    const rect = solutionDisplay.getBoundingClientRect();
            ...
            clearSolution() {
                document.getElementById('movesList').innerHTML = '<p>Click "Solve Automatically" to see the solution steps.</p>';
            }
