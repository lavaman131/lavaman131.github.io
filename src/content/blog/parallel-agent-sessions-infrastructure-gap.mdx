...
Claude Code shipped [Agent Teams](https://docs.anthropic.com/en/docs/claude-code/agent-teams) as a research preview in February 2026. (For the full technical breakdown of Agent Teams alongside Codex and Fleet Mode, see our [deep dive on Opus 4.6, Codex, and agent teams](/blog/opus-codex-agent-teams-deep-dive).) The architecture is straightforward: one session acts as a lead agent, spawning teammates that are each full Claude Code instances with their own context windows.
...
Cloud-based solutions (Codex cloud, Cursor cloud agents) get this for free because each agent runs in its own VM — see our breakdown of the [cloud VMs powering autonomous coding agents](/blog/cloud-vms-autonomous-agent-infrastructure) for how that isolation works under the hood. But cloud execution has latency, cost, and cold-start penalties. For local development — where iteration speed matters most — we have nothing.
