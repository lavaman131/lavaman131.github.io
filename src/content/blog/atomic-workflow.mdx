---
title: "Atomic: From AI Coding Infrastructure to Automated Procedures and Memory for AI Coding Agents"
date: "2025-12-01"
...
**Atomic** is that system. It's a curated workflow with focused agents and skills designed to work together.
...
flowchart TD
    subgraph Input[" "]
        TASK["Task + docs/notes/tickets"]
    end

    RC[Research Codebase Agent]
    TASK --> RC

    subgraph ResearchAgents[" "]
        CPF[Codebase Pattern Finder Agent]
        CL[Codebase Locator Agent]
        CA[Codebase Analyzer Agent]
        CRL[Codebase Research Locator Agent]
        CRA[Codebase Research Analyzer Agent]
        COR[Codebase Online Researcher Agent]
    end
    ...
    subgraph Output[" "]
        ...
        Tree["research/ tickets/ docs/ notes/"]
        ROut --- Tree
    end
    ...
    HSR((Human Spec Review))
    CS --> HSR

    CFL[Create Feature List Agent]
    HSR --> CFL

    subgraph FeatureOutput[" "]
        PROG((progress.txt))
        FLJSON((feature-list.json))
    end

    CFL --> PROG
    CFL --> FLJSON

    LOOP[for-each feature: compact + implement]
    FeatureOutput --> LOOP
    ...
    subgraph QA["Deterministic Test Hooks"]
        PRE["1. pre-commit checks"]
        LINT["2. linter checks"]
        TEST["3. tests"]
    end
    ...
    DBGRPT((Debug Report Output))
    DBG --> DBGRPT
    DBG -.-> IMP

    SUPP[Coding Agent]
    SUPP -.->|assists| ImplLoop

    classDef input fill:#2d2d2d,stroke:#666,stroke-width:2px,stroke-dasharray: 5 5,color:#fff
    classDef research fill:#2d4a3e,stroke:#28a745,stroke-width:2px,color:#fff
    classDef researchSub fill:#2d4a3e,stroke:#28a745,stroke-width:2px,stroke-dasharray: 5 5,color:#fff
    classDef output fill:#4a3d4a,stroke:#e91e8c,stroke-width:2px,stroke-dasharray: 5 5,color:#fff
    classDef spec fill:#3d3d5c,stroke:#6f42c1,stroke-width:2px,color:#fff
    classDef review fill:#4a3d4a,stroke:#e91e8c,stroke-width:2px,color:#fff
    classDef feature fill:#2d3d4a,stroke:#007bff,stroke-width:2px,color:#fff
    classDef featureOut fill:#2d3d4a,stroke:#007bff,stroke-width:2px,stroke-dasharray: 5 5,color:#fff
    classDef impl fill:#4a3d2d,stroke:#fd7e14,stroke-width:2px,color:#fff
    classDef implLoop fill:#4a3d2d,stroke:#fd7e14,stroke-width:2px,stroke-dasharray: 5 5,color:#fff
    classDef qa fill:#4a3d2d,stroke:#fd7e14,stroke-width:2px,stroke-dasharray: 5 5,color:#fff
    classDef debug fill:#2d4a4a,stroke:#17a2b8,stroke-width:2px,color:#fff
    classDef debugOut fill:#2d4a4a,stroke:#17a2b8,stroke-width:2px,stroke-dasharray: 5 5,color:#fff
    classDef support fill:#3d3d3d,stroke:#666,stroke-width:2px,color:#fff
    classDef annotation fill:none,stroke:none

    class TASK input
    class RC research
    class CPF,CL,CA,CRL,CRA,COR researchSub
    class ROut,Tree output
    class CS spec
    class HSR review
    class CFL feature
    class PROG,FLJSON featureOut
    class IMP,CHG,TST,VAL,MRK,CMT impl
    class PRE,LINT,TEST qa
    class DBG debug
    class DBGRPT debugOut
    class SUPP support
    class COMPACT1,COMPACT2,LOOP annotation
```
...
Six agents work simultaneously—this is where parallelization shines:
